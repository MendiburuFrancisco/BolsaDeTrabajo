...
const Login = () => {
  ...
  const handlerLogin = async (event) => {
    ...
    try {
      ...
      await signin(datos).then((res) => {
        
        console.log("A",isAuthenticated,res.status,"U :",user);
  ...
  return (
    <Layout className="login-card h-[calc(100vh-0px)]">
      ...
      <form onSubmit={handlerLogin} className="space-y-6 mx-6">
        {/* Email Input */}
        <div className="mb-4 ">
          <label
            htmlFor="email"
        ...
        <p
          id="mensaje-error"
          className="text-center text-red-500 font-semibold hidden"
        >
      ...
      <PrimaryLink
        to={"/singin"}
        text="¿Como obtengo una cuenta?"
        ariaLabel="Obtener una cuenta"
        type="submit"
      />
    </Layout>
  );
